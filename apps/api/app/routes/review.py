...
async def _run_builder(code: str, user_id: str) -> tuple:
...
async def _run_reviewer(
    code: str, builder_content: str, language: str, user_id: str
) -> tuple:
...
async def _process_review(review_id: str, code: str, language: str, user_id: str):
    ...
    try:
        ...
        baseline_future = _run_baseline(code, language, user_id)
        builder_future = _run_builder(code, user_id)
        ...
        issues_raw, summary, model_reviewer = await asyncio.wait_for(
            _run_reviewer(code, builder_content, language, user_id),
            timeout=45.0,
        )
